...
const ContactInfo* ContactList::GetContact(const string& nickname) const {
  std::map<string, ContactInfo>::const_iterator iter =
...
ContactList* ContactList::Load(const string& contacts_csv) {
  std::ifstream csvfile;
  csvfile.open(contacts_csv.c_str());
  ContactList* contact_list = new ContactList();
  contact_list->ParseCSV(&csvfile);
  csvfile.close();

  return contact_list;
}

static const char *readValue(const char* start, string* value) {
